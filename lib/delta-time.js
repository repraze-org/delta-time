...
const strict = /^\s*((([+-]?\s*[1-9][0-9]*(\.[0-9]+)?)|([+-]?\s*0(.[0-9]+)?))\s*[a-zμ]+\s*)+$/;
module.exports.strict = create(
    dt=>{
        if(!isString(dt)){
            throw new TypeError(`${dt} is not of type number or string`);
        }
        if(dt.trim() === "" || !dt.match(strict)){
